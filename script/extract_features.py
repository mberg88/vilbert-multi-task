...
class FeatureExtractor:
    ...
    def get_parser(self):
        ...
        parser.add_argument(
            ...
            type=float,
            default=0,
            help="Threshold of detection confidence above which " + 
            "boxes will be selected",
        )
        parser.add_argument(
            "--background",
            action="store_true",
            help="The model will output predictions for the background class " +
            "when set",
        )
        parser.add_argument(
    ...
    def _save_feature(self, file_name, feature, info):
        ...
        # from https://github.com/facebookresearch/vilbert-multi-task/...
        # ...issues/77#issuecomment-805822681
    ...
    def extract_features(self):
        ...
        if os.path.isfile(image_dir):
            ...
            # files = sorted(files)
            # files = [files[i: i+1000] 
            # for i in range(0, len(files), 1000)][self.args.partition]
            # load last count
            lastCntFil = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                      'extract_features_cnt__{}.txt'.format(
                                          os.path.basename(image_dir)))
            if os.path.isfile(lastCntFil):
                with open(lastCntFil, 'r') as cntR:
                    saved_str = cntR.read().replace('\n', '')
                    
                saved_cnt = int(saved_str)
            else:
                saved_cnt = 0
                
            # matchL = ['00001406', '00001411', '00002447', '00002454', 
            #          '00003453', '00003463']
            
            # add
            # if any(Path(img).stem in matchL for img in chunk):
            #   print('match found')
            # to handle matching imgs
                
            for cnt, chunk in enumerate(self._chunks(files, 
                                                     self.args.batch_size)):
                if cnt >= saved_cnt:
                    # update last count
                    with open(lastCntFil, 'w') as cntW:
                        cntW.write(str(cnt))
                    ...
                    try:
                        ...
                        print(f'base exception in chunk {chunk}')
                        continue

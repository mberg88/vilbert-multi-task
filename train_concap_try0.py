...
def main():
    ...
    parser.add_argument(
        "--distributed",
        action="store_true",
        help="whether use chunk for parallel training.",
    )
    parser.add_argument(
    ...
    for epochId in range(int(args.start_epoch), int(args.num_train_epochs)):
        ...
        # logger.info(dir(train_dataset.ds))
        # logger.info('len2')
        
        # test from 
        # https://github.com/tensorpack/tensorpack/issues/877...
        # #issuecomment-417160022
        # train_dataset.ds.reset_state()
        # logger.info(next(train_dataset.ds.get_data()))
        # TestDataSpeed(train_dataset.ds).start()

        for step, batch in enumerate(train_dataset):
            logger.info("  train step = %d", step)
        ...
        for step, batch in enumerate(validation_dataset):
            logger.info("  val step = %d", step)
            
            image_ids = batch[-1]
            logger.info('val image_ids found')
            
            batch = tuple(t.cuda(device=device, non_blocking=True) 
                          for t in batch[:-1])
            logger.info('val batch made')
            ...
            logger.info('val batch unpacked')

            batch_size = input_ids.size(0)            
            logger.info('val batch size found')
            ...
            logger.info('val model outputs found')

            masked_loss_v = masked_loss_v * args.img_weight
            loss = masked_loss_t + masked_loss_v + next_sentence_loss
            
            logger.info('val loss found')
        ...
        logger.info('val torch grad enabled')

...
class InputExample(object):
    ...
    def __init__(
        ...
        Args:
            ...
            tokens_a: string. The untokenized text of the first sequence. 
            For single
...
class ConceptCapLoaderTrain(object):
    ...
    Arguments:
        mode (str, required): mode of dataset to operate in, one of 
        ['train', 'val']
        ...
        drop_last (bool, optional): set to ``True`` to drop the last incomplete
            ...
            If ``False`` and the size of dataset is not divisible 
            by the batch size, then the last batch
            will be smaller. (default: False)
        cuda (bool, optional): set to ``True`` and the PyTorch tensors will get
            preloaded to the GPU for you (necessary because this lets us 
                                          to uint8 conversion on the 
            GPU, which is faster).
    """
    ...
    def __init__(
        ...
        print("train: LMDB loaded")
        
        ds = td.MapData(ds, lambda dp: dp if type(dp) is dict else None)
        print("train: LMDB lists removed")
        
        self.num_dataset = len(ds)
        ds = td.LocallyShuffleData(ds, cache)
        print("train: LMDB data shuffled")
        
        caption_path = os.path.join(corpus_path, "caption_train.json")
        # caption_path = os.path.join(corpus_path, "caption_val.json")
        
        # region_len is set to 36
        ...
        print("train: LMDB data prefetched")
        
        ds = td.MapData(ds, preprocess_function)
        print("train: LMDB data mapped")
        
        # self.ds = td.PrefetchData(ds, 1)
        ds = td.PrefetchDataZMQ(ds, num_workers)
        
        print('train: LMDB data ZMQ-prefetched')
        
        self.ds = td.BatchData(ds, batch_size)
        
        print('train: LMDB data batched')
        
        # self.ds = ds
        self.ds.reset_state()
        
        print('train: LMDB state reset')

        self.batch_size = batch_size
        self.num_workers = num_workers
        
        # for cnt, batch in enumerate(self.ds):
        #    print(f'try: {cnt}, {type(batch)}, {len(batch)}')

    def __iter__(self):
        print('train: running')
        # for cnt, batch in enumerate(self.ds.get_data()):
        for cnt, batch in enumerate(self.ds):
            print(f'train: cnt = {cnt}')
            input_ids, input_mask, segment_ids, lm_label_ids, is_next, \
                image_feat, image_loc, image_target, image_label, \
            ...
            g_image_loc = np.repeat(
                np.array([[0, 0, 1, 1, 1]], dtype=np.float32), batch_size, 
                axis=0
            )
...
class ConceptCapLoaderVal(object):
    ...
    single- or multi-process iterators over the dataset.
    Arguments:
        mode (str, required): mode of dataset to operate in, one of ['train',
                                                                     'val']
        batch_size (int, optional): how many samples per batch to load
            (default: 1).
        shuffle (bool, optional): set to ``True`` to have the data reshuffled
        ...
        batch,
            ...
            then the last batch
            will be smaller. (default: False)
        cuda (bool, optional): set to ``True`` and the PyTorch tensors 
        will get preloaded
            to the GPU for you (necessary because this lets us 
                                to uint8 conversion on the 
            GPU, which is faster).
    """
    ...
    def __init__(
        ...
        ds = td.LMDBSerializer.load(lmdb_file, shuffle=False)
        print("test: LMDB loaded")
        
        ds = td.MapData(ds, lambda dp: dp if type(dp) is dict else None)
        print("test: LMDB lists removed")
        ...
        ds = td.MapData(ds, preprocess_function)
        print("test: LMDB data mapped")
        
        self.ds = td.BatchData(ds, batch_size)
        print('test: LMDB data batched')
        
        self.ds.reset_state()
        print('test: LMDB state reset')
...
class BertPreprocessBatch(object):
    ...
    def __call__(self, data):
        ...
        # print(f"type(data['features']) = {type(data['features'])}")
        # print(f"shape(data['features']) = {data['features'].shape}")

        # image_feature_wp, image_target_wp, image_location_wp, num_boxes, \
        #    image_h, image_w, image_id, caption = (data)
        num_boxes = int(min(self.region_len, data['features'].shape[0]))
        
        # cut matrices
        image_feature_wp = data['features'][: num_boxes]
        image_target_wp = data['cls_prob'][: num_boxes]
        image_location_wp = data['boxes'][: num_boxes]       
        ...
        # print(f'image_feature_wp = {image_feature_wp}')
        # print(f'image_target_wp = {image_target_wp}')
        # print(f'image_location_wp = {image_location_wp}')
    ...
    def _truncate_seq_pair(self, tokens_b, max_length):
        ...
        # longer sequence
        # one token at a time. This makes more sense than truncating 
        # an equal percent
        # of tokens from each, since if one sequence is very short 
        # then each token
    ...
    def random_word(self, tokens, tokenizer, is_next):
        ...
        for i, token in enumerate(tokens):
            ...
            if prob < 0.15 and (not self.visualization):
                ...
                if prob < 0.8:
                    tokens[i] = tokenizer.convert_tokens_to_ids(
                                    tokenizer.mask_token)

                # 10% randomly change token to random token
                elif prob < 0.9:
                    tokens[i] = np.random.randint(len(tokenizer))
                    # torch.randint(len(tokenizer), labels.shape, 
                    # dtype=torch.long)
    ...
    def random_region(self, image_feat, image_loc, num_boxes, is_next, overlaps):
        ...
        for i in range(num_boxes):
            ...
            # if is_next == 1 and self.objective != 0:
            #     prob = 1 # if the target is unaligned mask, then not 
            # sample mask
            if prob < 0.15 and not self.visualization:
                prob /= 0.15
...
class ConceptCapLoaderRetrieval(object):
    ...
    Arguments:
        ...
        cuda (bool, optional): set to ``True`` and the PyTorch tensors 
        will get preloaded
            to the GPU for you (necessary because this lets us to uint8 
                                conversion on the 
            GPU, which is faster).
    """
    ...
    def __init__(
        ...
        if not os.path.exists(lmdb_file):
            lmdb_file = "/coc/pskynet2/jlu347/multi-modal-bert/data/" + \
                "conceptual_caption/validation_feat_all.lmdb"
        caption_path = "/coc/pskynet2/jlu347/multi-modal-bert/data/" + \
                "conceptual_caption/caption_val.json"
    ...
    def __iter__(self):
        ...
        for index in range(self.__len__()):
            ...
            caption, input_mask, segment_ids, txt_image_id, caption \
                = self._entry[caption_idx]
...
class BertPreprocessRetrieval(object):
    def __init__(
        ...
        self.visual_target = visual_target
        self.num_caps = data_size
        self.captions = list(json.load(open(caption_path, 
                                            "r")).values())[:data_size]

    def __call__(self, data):

        image_feature_wp, image_target_wp, image_location_wp, num_boxes, \
            image_h, image_w, image_id, caption = (
            data
        )
    ...
    def convert_example_to_features(
        ...
        """
        Convert a raw sample (pair of sentences as tokenized strings) 
        into a proper training sample with
        IDs, LM labels, input_mask, CLS and SEP tokens etc.
        :param example: InputExample, containing sentence input as strings 
        and is_next label
        :param max_seq_length: int, maximum length of sequence.
        :param tokenizer: Tokenizer
        :return: InputFeatures, containing all inputs and labels of one sample 
        as IDs (as used for model training)
        """
        image_feat = example.image_feat
    ...
    def _truncate_seq_pair(self, tokens_b, max_length):
        ...
        # This makes more sense than truncating an equal percent of tokens
        # from each, since if one sequence is very short then each token
        # that's truncated likely contains more info than a longer sequence.
        while True:
            total_length = len(tokens_b)
            if total_length <= max_length:

...
class FlickrGroundingDataset(Dataset):
    def __init__(
        ...
        print(f'len(image IDs) = {len(self._image_features_reader._image_ids)}')
        print(f'len(gt image IDs) = {len(self._gt_image_features_reader._image_ids)}')
        
        print('len(symm diff) = {}'.format(len(set(self._image_features_reader._image_ids) -
                                            set(self._gt_image_features_reader._image_ids))))
    ...
    def _load_annotations(self, clean_datasets):
        ...
        with open(
            os.path.join(
                # "/checkpoint/vedanuj/datasets/flickr30k",
                self.dataroot,
                "%s.txt" % self.split
            ),
            "r",
        ...
        for img in images:
            ...
            for i, sent in enumerate(sentences):
                for phrase in sent["phrases"]:
                    # if datasets are cut check if entry from uncut dataset is in
                    # cut dataset
                    # Here img comes from the uncut dataset
                    # whereas self._image_features_reader comes from the cut dataset
                    if str(phrase["phrase_id"]) in annotation["boxes"].keys() and \
                        img in self._image_features_reader._image_ids:
                        entries.append(
                            {
        ...
        print(f'len(sentences) = {sent_cnt}')
        print(f'len(annotations) = {note_cnt}')
    ...
    def __getitem__(self, index):
        ...
        #    print(f'key = {entry_k}, val = {entry_v}')
